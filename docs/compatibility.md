...
| Notion native app | Persistent-client new-page workflow, runtime menu/title/body selectors, detached-document pruning, stable title/body writes, and final value search | AX and PID background delivery; one bounded foreground fallback is available only after a native no-effect proof | Exact title and body values remain present in a fresh final observation | Timed write path passed in 3.95 s; full gate budget is 10 s (`npm run test:notion-workflow-live`) |
