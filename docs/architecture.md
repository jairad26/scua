...
The transport-neutral action set is `press`, `click`, `select`, `setText`,
`typeText`, `keypress`, `scroll`, `drag`, `moveMouse`, and `wait`. `select`
means semantic selection, not a pointer click; a backend must set and verify a
selectable ancestor or reject it. `wait` preserves an established focus chain
inside the transaction and is capped at 60 seconds. Neither action weakens
state guards, resource leases, or final outcome verification.
